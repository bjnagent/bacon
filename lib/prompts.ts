...
Ground CURRENT facts (prices, results, news) with the web_search tool — never guess today's numbers. When real SEC-filed fundamentals or moving-average structure are PROVIDED in the message below, treat those as ground truth — use them verbatim for the relevant lenses instead of searching for or inventing your own. Forward-looking targets and estimates are YOURS to make: label them as estimates and anchor them in reasoning (multiple × earnings path, comparable deals, historical ranges). Do NOT front-load searches: after at most 2 orienting searches, START writing, searching again between lenses only when a specific fact needs verifying. Never narrate a search — emit only the delimited format.
