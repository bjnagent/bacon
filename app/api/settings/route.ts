...
export async function GET() {
  ...
  const { data, error } = await sb.from("settings").select("scout_interval_minutes,last_sweep_at,news_source,news_focus").eq("user_id", user.id).maybeSingle();
...
export async function PATCH(req: Request) {
  ...
  let body: { scout_interval_minutes?: number; news_source?: string; news_focus?: string };
